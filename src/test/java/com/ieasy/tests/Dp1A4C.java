...
public class Dp1A4C extends TestBase{
private static final Logger log = LogManager.getLogger(Dp2APlan.class);
		...
		for (int i = 1; i < rows; i++) {
			...
			for (int j = 0; j < columns; j++) {
				...
				if (sheet.getCellData(sheetName, j, 0).equalsIgnoreCase("PinCode")) {
					 ...
					 * 0).equalsIgnoreCase("Daughter2 Age")) {
					 * hPojo.setDaughter2Age(sheet.getCellData(sheetName, j, i)); } else if
					 * (sheet.getCellData(sheetName, j, 0).equalsIgnoreCase("Son2 Age")) {
	...
	public Object[][] healthDataProvider() throws Exception {
		...
		ArrayList<HealthPOJO> healthDataList = testData(xlFilePath, "Dp1A4C");
	...
	public void dP1A4C(HealthPOJO pojoData) throws InterruptedException {
